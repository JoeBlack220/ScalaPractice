...
object Anagrams {
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] =
    dictionary.map(w => (w, wordOccurrences(w))).groupBy(x => x._2)
    .map(x => (x._1, x._2.map(y => y._1))).withDefault(_ => Nil)
   ...
   */
  def sub1c(occ_cur: Occurrences, ch: Char): Occurrences = {
    val occs = for((letter, count) <- occ_cur) yield
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences :: (for(occ <- occurrences) yield combinations(sub1c(occurrences, occ._1))).flatten.distinct
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    if(y.length == 0) x
      ...
      subtract(sub1c(x, y.head._1), sub1c(y, y.head._1))
    }
  }
    ...
    def helper(occ: Occurrences): List[Sentence] = {
      if(occ.isEmpty) List(List())
        ...
        for{
          ...
          anagrams <- helper(subtract(occ, combination))
        } yield word :: anagrams
      }
    }
