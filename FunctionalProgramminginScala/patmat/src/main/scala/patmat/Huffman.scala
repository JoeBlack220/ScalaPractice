...
object Huffman {
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs match {
      case List() => List()
      case (c, w) :: ys => insert(new Leaf(c, w), makeOrderedLeafList(ys))
    }

//  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortWith(_._2 < _._2) map { case (char, weight) => Leaf(char, weight) }


  def insert(cur: Leaf, xs: List[Leaf]): List[Leaf] = xs match {
      case List() => List(cur)
      case y :: ys => if(cur.weight < y.weight) cur :: xs else y :: insert(cur, ys)
    }
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] =
      ...
      else (makeCodeTree(trees(0), trees(1)) :: trees.drop(2)).sortWith((a, b) => weight(a) < weight(b))
