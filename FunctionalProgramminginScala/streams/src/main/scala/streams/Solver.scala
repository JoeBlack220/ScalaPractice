...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    case hd #:: res =>
      if(explored.contains(hd._1)) newNeighborsOnly(res, explored)
      else hd #:: newNeighborsOnly(res, explored)
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case(b, ms) #:: rest =>
      if(done(b)) Stream.empty
        ...
        val nextSet = explored + b
        val next = newNeighborsOnly(neighborsWithHistory(b, ms), nextSet)
        next ++ from(rest ++ next, nextSet)
      }
  }
