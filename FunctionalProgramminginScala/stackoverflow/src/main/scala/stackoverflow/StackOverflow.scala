...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    ...
    val answers = postings.filter(_.postingType == 2).map(x => {
      x.parentId match {
        case Some(id) => (id, x)
        case None => (x.id, x)
      }
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    grouped.map(x => (x._2.head._1, answerHighScore(x._2.map(item => item._2).toArray)))
  }
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.map(x => (firstLangInTag(x._1.tags, langs), x._2)).filter(x => !x._1.isEmpty).map(x => {
      x._1 match {
        case Some(index) => (index * langSpread, x._2)
      }
    }).persist()
  }
